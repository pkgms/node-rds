...
class PgQuery {
    ...
    constructor(isBeginTransaction = false) {
        this._getServer(isBeginTransaction);
    }
    ...
    async get(num, sql = false) {
        ...
        if (sql) {
            ...
            if (len > 0) {
                for (let i = 1; i <= len; i++) {
                    sql = sql.replace(' $' + i + ' ', ` \'` + values[i - 1] + `\' `)
                        .replace('$' + i + ';', `\'` + values[i - 1] + `\';`)
                        .replace('$' + i + ',', `\'` + values[i - 1] + `\',`);
                }
            }
            return sql;
        ...
        if (this.options._method === 'SELECT' && num !== 1) {
            ...
            if (Array.isArray(res) && res.length > 0) {
                ...
                this._server.origin = JSON.parse(JSON.stringify(originObj));
    ...
    where(key, op, value) {
        ...
        if (OP_ACCEPT.indexOf(op) < 0) {
            throw new Error('Build query with invalid operation');
        }
        let whereIndex = this.options._whereValues.length;
        if (Array.isArray(value)) {
            if (op === 'between' || op === 'not between') {
                this.options._wherePlaceholder.push(`"${key}" ${op.toLocaleUpperCase()} $${++whereIndex} AND $${++whereIndex}`);
            ...
            this.options._wherePlaceholder.push(`"${key}" ${op} $${++whereIndex}`);
        ...
        for (let v of arr) {
            ...
            for (let key in v) {
                if (v.hasOwnProperty(key)) {
                    ...
                    insert.push(`$${this.options._values.length + 1}`);
    ...
    async update(obj, sql = false) {
        ...
        const originObj = this._server.origin;
        ...
        for (let key in obj) {
            if (obj.hasOwnProperty(key)) {
                ...
                this.options._updatePlaceholder.push(`"${key}" = $${index}`);
    ...
    join(type = 'INNER', table, foreignKey, op, ownerKey) {
        ...
        this.options._joinPlaceholder += ` ${type} JOIN "${table}" ON ${foreignKey} ${op} ${ownerKey}`;
    ...
    build() {
        ...
        switch (this.options._method) {
            ...
            case 'INSERT':
                ...
                query += `INSERT INTO "${this.options._table}" (${this.options._insertFields.join(',')}) VALUES ${insertArr.join(',')} RETURNING *`;
                break;
            case 'UPDATE':
